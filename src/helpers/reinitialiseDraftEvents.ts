      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === "edit_requirements") {
          return i.reply({
            ...
            if (i.customId === "get_event_id") {return i.reply({content: `This event's ID is: \`${event.id}\``,flags: MessageFlags.Ephemeral,
            });
        }
        
        if (i.customId === "publish_event") {
          // Only allow the host to publish
          if (i.user.id !== event.hostId) {return i.reply({content: "❌ Only the host can publish this event.",flags: MessageFlags.Ephemeral,
            });
          }
